...
Cuando ejecutas `java Main`, la JVM arranca y crea sus áreas de memoria (esto pasa UNA sola vez por aplicación):
...
lo lee, y guarda su estructura en el Metaspace. Sin esto, la JVM no sabría qué campos o métodos tiene tu clase.
...
1. **Interpretación**: lee el bytecode instrucción por instrucción y
   la ejecuta al vuelo. Arranque rápido, pero más lento en el largo plazo porque vuelve a interpretar la misma instrucción cada vez.

2. **JIT (Just-In-Time Compiler)**: mientras el programa corre, la JVM
   detecta qué métodos se ejecutan muchísimas veces ("hot code") y los compila directamente a código nativo de tu CPU física, guardando ese resultado en caché para no reinterpretar cada vez.

   > Esto explica por qué una aplicación Java a veces "tarda en calentar"
   > al inicio, pero después de unos segundos/minutos corre mucho más rápido.
...
Execution Engine ejecuta:
   ...
   └── JIT compila a nativo el código "caliente"
